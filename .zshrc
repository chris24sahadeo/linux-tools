# @chris24sahadeo: Delete the .zshrc that the ohmyzsh script installs and create a symlink to this file.s
...
# colorize  # Does not seem to work? Colorizes cat output.
# copydir  # No longer exists? Copies the pwd to the clipboard.
# Sat Feb 25 03:32:23 PM AST 2023 stopped looking though https://github.com/ohmyzsh/ohmyzsh/wiki/Plugins-Overview at the `copyfile` plugin, pick up from there...
plugins=(
  aliases           # Type `acs` to use an `acs -h` for help.
  cp                # cp with progress bar (rsync). Use command `cpv`
